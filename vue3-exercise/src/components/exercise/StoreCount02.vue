...
const add = () => {
    ...
    // 访问全局命名空间的mutation：store.commit("文件名/你定义的方法名")
...
const sunAsync = () => {
    store.dispatch("count02/decrementAsync")
}
</script>
<template>
    <!-- 访问全局命名空间的state:$state.state.自己定义的变量名 -->
    <h5>全局状态全局命名空间计步器：{{$store.state.count}}</h5>
    <!-- 访问局部命名空间的state：store.state.modules定义的名字(modules文件的名字).你定义的变量名 -->
    <h5>全局状态module中的02计步器：{{$store.state.count02.count}}</h5>
